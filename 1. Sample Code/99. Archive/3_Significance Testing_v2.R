...
for (i in 1:6){
  for (j in 1:6){
    if (j>i){
      IP_significance[i,j] = round(DM.test(eval(as.name(all_IP[i])),eval(as.name(all_IP[j])),(datMxTest  %>% filter(ymd >= filter_test))$todFrom1,h=1,
...
all_IP_resid = c("IP_HR_gauss_error","IP_HRFCNN_error",
                 "IP_LR_ocat_error","IP_LRFCNN_error",
                 "IP_MR_ocat_error","IP_MRCNN_error"
)
...
loss = function(resid){return(pmin(abs(resid),2))}
...
for (i in 1:6){
  for (j in 1:6){
    if (j>i){
      ...
      mean_difference = mean( loss(eval(as.name(all_IP_resid[i]))) - loss(eval(as.name(all_IP_resid[j]))), trim=trim )
      
      library(boot)
      out = tsboot(loss(eval(as.name(all_IP_resid[i]))) - loss(eval(as.name(all_IP_resid[j]))), 
                   statistic = function(x) mean(x, trim=trim), R = 500, l = round(n/24), sim = "fixed")
      
      mean_sample_centered = out$t-mean(out$t)
      lower_bound = quantile(mean_sample_centered,0.05)
      upper_bound = quantile(mean_sample_centered,0.95)
      
      IP_significance[i,j] = between(mean_difference,lower_bound,upper_bound)
      IP_significance[j,i] = IP_significance[i,j]
    }
  }
}
...
significance_formatter = formatter("span", 
                                   style = x ~ style(color = ifelse(x == TRUE, "red", 
                                                                    "green")))
...
metrics_box(all_IP_gams,change_unit=FALSE, loss=loss_truncated_mae,loss_name="Truncated AE",output="IP")


simple_histo(all_IP_gams,change_unit=FALSE, loss=loss_truncated_mae,loss_name="Truncated AE",output="IP")



simple_histo(all_IP_gams,change_unit=FALSE, loss=loss_mae,loss_name="Absolute Errors [half-hour]",output="IP")
simple_histo(all_IP_gams,change_unit=FALSE, loss=loss_mse,loss_name="Squared Errors [half-hour²]",output="IP")
...
metrics_box(all_IP_gams,change_unit=FALSE, loss=loss_mbe,loss_name="Bias Errors [half-hour]",output="IP")
metrics_box(all_IP_NNs,change_unit=FALSE, loss=loss_mbe,loss_name="Bias Errors [half-hour]",output="IP")
